...
from app import db
from app.models import Reservation, ComplianceConfig, QuesturaLog
from app.services.questura import QuesturaService, QuesturaGuest, get_questura_service
from app.services.tourist_tax import TouristTaxService, get_tax_service, generate_monthly_tax_report
...
def submit_questura_daily(self):
    ...
    try:
        ...
        if not service.is_configured():
            return {
                'success': False,
                'error': 'Questura service not configured'
            }
...
def retry_failed_questura(self, reservation_ids: List[int] = None):
    ...
    try:
        query = Reservation.query.filter(
            Reservation.questura_status == 'rejected'
        )
        if reservation_ids:
...
def generate_monthly_tourist_tax_report(self):
    ...
    try:
        ...
        logger.info(f"Tourist tax report generated: {report['total_reservations']} reservations, €{report['total_tax']:.2f}")
...
def send_guest_checkin_reminder(self):
    ...
    try:
        ...
        if not_ready:
            # Send email to admin
            guest_list = '\n'.join([
                f"  #{r.id}: {r.guest_name} - {r.check_in} (missing: {_missing_fields(r)})"
                for r in not_ready
            ])
        ...
        return {
            'success': True,
            'total_checkins': len(reservations),
            'incomplete': len(not_ready)
        }
...
def _missing_fields(reservation: Reservation) -> List[str]:
    ...
    missing = []
    if not reservation.guest_surname: missing.append('surname')
    if not reservation.guest_first_name: missing.append('first_name')
    if not reservation.guest_birth_date: missing.append('birth_date')
    if not reservation.guest_birth_place: missing.append('birth_place')
    if not reservation.guest_nationality: missing.append('nationality')
    if not reservation.guest_document_type: missing.append('document_type')
    if not reservation.guest_document_number: missing.append('document_number')
    if not reservation.guest_document_expiry: missing.append('document_expiry')
    if not reservation.guest_document_country: missing.append('document_country')
    if not reservation.guest_gender: missing.append('gender')
    return missing
