...
def sync_feed(feed: ICalFeed) -> tuple[int, int]:
    ...
    # IMPORTANT: never cancel past stays (check_out < today) — the feed only
    # contains future dates, so past reservations would otherwise be orphaned
    # and incorrectly marked cancelled, disappearing from History.
    cancelled = 0
    ...
    for r in orphans:
        # Double-guard: skip past stays even if query slips (defense in depth)
